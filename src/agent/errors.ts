...
export function agentFailure(error: unknown) {
  ...
  if (statuses.some(status => status === 401 || status === 403)) return { names, statuses, status: 503, message: 'El servicio del modelo rechazó sus credenciales. El administrador debe revisar la configuración de AI Gateway; tu token de consola no es el problema.' };
